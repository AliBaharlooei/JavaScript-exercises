...
let titleArray, dateArray;

//=> check key value in local storage not null
let checkEmptyArray = ()=>{    
    if(!(localStorage.getItem("title") && localStorage.getItem("date"))){
        ...
        titleArray = localStorage.getItem("title").split(',');
        dateArray = localStorage.getItem("date").split(',');
    }
}
checkEmptyArray();

//=> save task in the local storage
let saveTask = (title,date)=>{
    titleArray.push(title)
    localStorage.setItem("title",titleArray);
    dateArray.push(date);
    localStorage.setItem("date",dateArray);
}
let showTitleValue = ()=>{    
    let lastTaskValue;
    if(localStorage.getItem("title")){
        let getTaskArray = localStorage.getItem("title").split(",");
        let lastTaskArray = getTaskArray.length - 1;
        lastTaskValue = getTaskArray[lastTaskArray]
    }
    return lastTaskValue
}
console.log(showTitleValue());

let showDateValue = ()=>{    
    let lastDateValue;
    if(localStorage.getItem("date")){
        let getDateArray = localStorage.getItem("date").split(",");
        let lastDateArray = getDateArray.length - 1;
        lastDateValue = getDateArray[lastDateArray]
    }
